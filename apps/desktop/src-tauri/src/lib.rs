...
#[tauri::command]
fn open_model_directory(model_id: String, state: State<'_, HostState>) -> Result<(), HostError> {
    let directory = state
        .0
        .model_directory_target(&model_id)
        .ok_or_else(|| HostError::new("host.model_unknown", "model is not in the local catalog"))?;
