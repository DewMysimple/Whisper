...
for (const theme of ['light', 'dark'] as const) {
  test(`output locations share the card style and keep paths inline (${theme})`, async ({
    page,
  }, testInfo) => {
    ...
    await panel.screenshot({ path: testInfo.outputPath(`folders-${theme}.png`) });
    ...
    await expect(panel.locator('.output-setting-card')).toHaveCount(4);
    await expect
      .poll(() =>
        page.evaluate(
          () =>
    ...
    await panel.screenshot({ path: testInfo.outputPath(`custom-long-path-${theme}.png`) });
