...
export function ModelSwitchView() {
  const workspaceTab = useWorkspace((state) => state.modelWorkspaceTab);
  const setWorkspaceTab = useWorkspace((state) => state.setModelWorkspaceTab);
  ...
  return (
    <div className="model-workspace">
      <div className="model-workspace-tabs" role="tablist" aria-label="模型工作台">
        <button
          aria-controls="model-library-workspace"
          aria-selected={workspaceTab === 'library'}
          className={workspaceTab === 'library' ? 'is-active' : ''}
          id="model-library-tab"
        ...
        >
          ...
          <span>
            <strong>模型选择</strong>
            <small>本地库存与默认模型</small>
          </span>
        </button>
        <button
          aria-controls="model-parameter-workspace"
          aria-selected={workspaceTab === 'parameters'}
          className={workspaceTab === 'parameters' ? 'is-active' : ''}
          id="model-parameter-tab"
      ...
      {workspaceTab === 'library' ? (
        <div
          className="model-library-workspace"
          id="model-library-workspace"
          role="tabpanel"
          aria-labelledby="model-library-tab"
        >
          ...
          <section className="panel model-library" aria-labelledby="model-library-title">
            ...
            <aside className="model-roadmap-note">
              <span className="model-install-dot is-missing" aria-hidden="true" />
              <div>
                <strong>自定义模型尚未开放</strong>
                <small>当前版本只读取受管理的 Large V3 系列模型，不加载外部目录。</small>
              </div>
        ...
        <div
          className="model-parameter-workspace"
          id="model-parameter-workspace"
          role="tabpanel"
          aria-labelledby="model-parameter-tab"
        >
