...
`OutputPanel` 以四个连续分区展示保存位置、格式、同名文件与完成动作；外观只在 `output-panel.css` 维护，受 `check:styles` 约束。分段按钮与格式 checkbox 使用稳定命中区域、主题变量和原生键盘语义；无选中格式时的说明位于标题下、选项上。保存策略、自定义目录选择和输出规则继续调用既有 workspace 动作，页面不新增路径事实源。
