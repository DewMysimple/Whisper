...
| `PreferenceChoiceCard` | 偏好设置的主题与界面大小单选卡片，保留原生 radio 语义 | `preference-choice-card.css` |
...
- `Button` 的主按钮可用 `surface="flat"` 选择纯色、无渐变和投影的外观；禁用时显示中性灰，仍保留原生禁用语义与键盘焦点。开始转录另外以低对比度边框、播放图标框和快捷键键帽细化层次：按钮表面规则位于 `button.css`，内容布局规则位于页面样式；默认外观及偏好设置动效预览不变。
...
- `PreferenceChoiceCard` 统一偏好设置中两组预览卡的表面、选中态和键盘焦点；主题与整体界面大小提供各自预览，大小卡还显示可选说明。整体大小面板位于字体设置块下方并占满横向宽度；预设一次设置框架、工作台、日志字号和工作台内容宽度，卡片选中状态由四项实际值推导，单独微调后不保留虚假的预设状态。
...
配置分组默认裁切内部网格以保持面板圆角，只有 `RoundedSelect` 展开时临时允许菜单越过卡片；底行字段和网格仍须保留圆角。长参数页与短模型页切换时，根滚动容器通过稳定滚动条槽保持内容水平位置。新增分组或调整列数时，应同时检查 3／2／1 列末行圆角、跨卡片菜单的命中，以及两页切换的左边缘位置。

`OutputPanel` 以四个连续分区展示保存位置、格式、同名文件与完成动作；外观只在 `output-panel.css` 维护，受 `check:styles` 约束。分段按钮与格式 checkbox 使用稳定命中区域、主题变量和原生键盘语义；唯一格式要求提示常驻在选项下方，不再展示额外灰色说明，勾选切换不改变纵向位置。各分区分隔线延伸至面板内沿。保存策略、自定义目录选择和输出规则继续调用既有 workspace 动作，页面不新增路径事实源。

`LaunchCard` 将媒体进度入口、开始转录主按钮和可选状态说明收在 `.launch-action-dock` 内；完整边框操作卡在双列工作台与左侧“模型与参数”入口卡同顶同高。卡内并列两个铺满高度的操作面板，媒体进度作为窄幅次操作，转录作为宽幅主操作；两者共用内边距、圆角和对齐线。无输入时进度入口仍显示但禁用，避免媒体状态切换推动布局。未选择输出格式时，启动区不重复显示格式提示；输出设置区继续负责说明。其他执行阻塞原因使用卡片内状态行，并通过 `aria-describedby` 关联主按钮。清单标题独占下一行并使用完整宽度，状态变化不改变标题或操作区几何位置。
