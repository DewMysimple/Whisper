...
`LaunchCard` 将媒体进度入口和开始转录主按钮放在同一个 `.launch-action-dock` 操作区；无输入时进度入口仍显示但禁用，避免媒体状态切换使操作区跳动。清单标题独占下一行并使用完整宽度，清单状态变化不改变标题位置或换行。执行条件说明位于操作区下方，由 `.launch-summary` 保留两行文字空间；条件满足时清空文字并隐藏可访问性状态，避免格式勾选导致主按钮纵向移动。
