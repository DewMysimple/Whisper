...
`OutputPanel` 以四个连续分区展示保存位置、格式、同名文件与完成动作；外观只在 `output-panel.css` 维护，受 `check:styles` 约束。分段按钮与格式 checkbox 使用稳定命中区域、主题变量和原生键盘语义；无选中格式时的说明位于标题下、选项上；隐藏时保留布局占位，确保勾选切换不改变格式选项与下方设置的纵向位置。保存策略、自定义目录选择和输出规则继续调用既有 workspace 动作，页面不新增路径事实源。

`LaunchCard` 的执行条件说明位于提交按钮下方，由 `.launch-summary` 保留两行文字空间；条件满足时清空文字并隐藏可访问性状态，避免格式勾选导致提交按钮纵向移动。
