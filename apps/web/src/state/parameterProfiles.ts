...
export type ParameterProfileKey = `${ModelId}:${PresetId}`;
...
export function parameterProfileKey(modelId: ModelId, presetId: PresetId): ParameterProfileKey {
  return `${modelId}:${presetId}`;
}

export function parseParameterProfileKey(value: string): [ModelId, PresetId] | null {
  const [modelId, presetId, extra] = value.split(':');
  if (
    extra !== undefined ||
    !MODEL_IDS.includes(modelId as ModelId) ||
    !PRESET_IDS.includes(presetId as PresetId)
  ) {
    return null;
  }
  return [modelId as ModelId, presetId as PresetId];
}
