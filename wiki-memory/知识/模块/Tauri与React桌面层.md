...
- `TaskList.tsx` 的展开历史把任务卡收纳在独立归档区域，全屏四列并响应降为 3／2／1 列；归档标题不重复说明任务数和列数，管理区也不显示“查找与筛选”说明或可见／结果计数。状态图标、名称与日期、百分比位于同一三列标题行；名称中间列可压缩，主体与扩展名分开布局，仅主体省略，因而不会挤动右侧进度。历史事实区首行使用与统计带同轴的三列网格：转录模式位于第一列，TXT／MD／SRT 在第二列以普通正文显示，第三列根据任务快照显示“默认参数”或“自定义参数”。媒体／耗时／媒体时长继续位于下一行三列；事实区横线连接卡片内容面两端，配置和统计列线均贯通上下横线，不建立展示状态。`TaskMonitor.tsx` 把元信息、两类进度及输入／输出信息组成连续信息面，使用列线而非独立小卡划分内容；各行横线连接主卡左右边界，元信息列线贯通行高，两列区域中心线同轴，两类进度通过统一说明区高度保持轨道同轴。当前媒体的状态和位置只在逐媒体列表维护，主监控不重复展示。
