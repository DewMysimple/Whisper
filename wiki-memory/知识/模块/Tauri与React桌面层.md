...
- `TaskList.tsx` 的展开历史把任务卡收纳在独立归档区域，全屏四列并响应降为 3／2／1 列；归档标题不重复说明任务数和列数。状态只由原有图标表达，图标保留可访问名称；格式标签使用独立的 28px 胶囊位于图标右侧。“参数已调整”只说明相对 Preset 存在参数覆盖。长名称把主体与扩展名分开布局，仅主体允许中间截断。转录模式和媒体／耗时／媒体时长三项统计均从任务快照派生；事实区横线连接卡片内容面两端，统计列线贯通整行，不建立展示状态。`TaskMonitor.tsx` 把元信息、两类进度及输入／输出信息组成连续信息面，使用列线而非独立小卡划分内容；各行横线连接主卡左右边界，元信息列线贯通行高，两列区域中心线同轴，两类进度通过统一说明区高度保持轨道同轴。当前媒体的状态和位置只在逐媒体列表维护，主监控不重复展示。
