...
- `TaskList.tsx` 的展开历史把任务卡收纳在独立归档区域，全屏四列并响应降为 3／2／1 列；格式标签、转录模式和三项同排统计均从任务快照派生，不建立展示状态。`TaskMonitor.tsx` 把元信息、两类进度及输入／输出信息组成连续信息面，使用列线而非独立小卡划分内容；当前媒体的状态和位置只在逐媒体列表维护，主监控不重复展示。
