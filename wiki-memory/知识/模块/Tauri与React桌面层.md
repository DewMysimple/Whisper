...
- `PreferenceChoiceCard.tsx` 与对应 CSS 是偏好设置中主题卡和整体界面大小卡的公共入口，使用原生 radio 保留单选键盘语义。`appearancePreferences.ts` 定义偏小／平衡／偏大三组三轴字号；默认值引用平衡。`workspace.ts` 原子更新已有的框架、内容和日志字号；卡片由三项实际值推导选中态，微调后为自定义组合，无新增持久化字段。
